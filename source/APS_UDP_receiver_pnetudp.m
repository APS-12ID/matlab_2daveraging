function APS_UDP_receiver_pnetudp(port)
...
while isrun
    ...
    %msg = judp('receive', port, 1000, 1000); 
    %len = length(data);
    if len>0
...
% if a syntax is 'file dir somthingelse'..
        ...
        switch lower(DET)
            ...
            case 'eiger9m'
                ...
                imgname = [bd, ext];
%                cmd = sprintf('updateH5 %s %0.3f %0.4f %0.2f %0.2f %0.2f %0.2f %0.2f %0.2f %s', ...
%                    fullfilen,  
%                system(cmd)
            otherwise
                [~, bd, ext] = fileparts(filen);
        ...
        %%% Azimuthal average
% SAXS["Temperature"], SAXS["Exposuretime"], SAXS["BS"], SAXS["IC"], SAXS["SAXSBS"], SAXS["CenterBS"], SAXS["GISAXSBS"], SAXS["GIWAXSBS"]        
        %pSpace
        %data
        ...
        if ~exist(datafullname, 'file')
            not_available = true;
            t0 = now;
            while not_available && (now-t0)*3600*24<filewait_timeout
                pause(0.02)
                not_available = ~exist(datafullname, 'file');
            end
            if not_available
                fprintf('%s has not been saved in %0.1f seconds.\n', filen, filewait_timeout)
                continue;
            end
        end
        tic;
...
%            tic;
%            datafullname = fullfile(workingDir, filesep, filen);
%            %datafullname = fullfile(avg.folder, filesep, filen);
% %            newcmd = sprintf('/home/beams15/S12IDB/bin/updateH5 %s %0.3f %0.4f %0.2f %0.2f %0.2f %0.2f %0.2f %0.2f %s', ...
% %                datafullname, avg.temperature, avg.expt, avg.photodiode,...
% %                avg.I0ofsample, avg.SAXSBS, avg.centerBS, avg.GISAXSBS,...
% %                avg.GIWAXSBS, datafullname);
% %            disp(newcmd);
% %            unix(newcmd);
%            updateH5att_12IDB(datafullname, [avg.temperature, avg.expt, avg.photodiode,avg.I0ofsample, avg.SAXSBS, avg.centerBS, avg.GISAXSBS,avg.GIWAXSBS]);
%            t2=toc;
%            fprintf('It takes %.5f sec to update.\n', t2);
%         end
...
%         elseif numel(pSpace) == 5
%             avg.photodiode = str2double(data(pSpace(2)+1:pSpace(3)-1));
%             avg.I0ofsample = str2double(data(pSpace(3)+1:pSpace(4)-1));
%             avg.expt = str2double(data(pSpace(4)+1:pSpace(5)-1));
%             avg.energynew = str2double(data(pSpace(5)+1:length(data)));
%         end

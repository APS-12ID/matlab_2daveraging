...
function SAXSLee_12IDB_data_autotransfer_start(varargin)
    ...
    function autoaverage(datadir, datafilen, savefilen, logfilen, usePhd)
        ...
        dataNorm=[data(:,1) data(:,2:3)/normNum data(:,4:end)];
    ...
    function a = runcmd(cmd)
        ...
        absfilenAveragedIc = fullfile(np, 'AveragedIc', savefilen);
        fileisnotthere = 1;
        timeout = 50;
        fid = -1;
        ...
        if ~isempty(fid)
            autoaverage(np, absfilen, absfilenAveraged,   extrLog(absfilen),1);
            autoaverage(np, absfilen, absfilenAveragedIc, extrLog(absfilen),0);
        end
    end
